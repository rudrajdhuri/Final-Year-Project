...
# Increased from (1280, 720) → (1920, 1080) for better clarity.
# Pi cam v2 supports up to 3280x2464 but 1920x1080 is the sweet spot —
# fast enough for near-instant capture, sharp enough for model input.
_PI_CAPTURE_SIZE = (1920, 1080)


def _normalize_picamera_frame(frame):
    """
    Picamera2 returns RGB even when BGR888 is requested on some firmware.
    Always convert to BGR so OpenCV and the models see correct colours.
    """
    if frame is None or getattr(frame, "size", 0) == 0:
        raise RuntimeError("Pi camera returned an empty frame")
...
def _open_camera():
    ...
    try:
        ...
        config_kwargs = {
            "main": {"size": _PI_CAPTURE_SIZE, "format": "BGR888"},
        }
        if transform is not None:
            config_kwargs["transform"] = transform
        ...
        # Clarity controls — applied before start() so they take effect on first frame
        camera.set_controls({
            "Sharpness":    2.0,    # 0.0 (soft) to 16.0; 2.0 gives noticeably crisper edges
            "Contrast":     1.2,    # 0.0 to 32.0; slight boost makes details pop
            "Brightness":   0.0,    # -1.0 to 1.0; 0.0 = neutral, adjust if scene is dark
            "Saturation":   1.2,    # 0.0 to 32.0; slight boost for better colour separation
            "NoiseReductionMode": 2, # 0=off 1=fast 2=high quality
            "AwbEnable":    True,   # auto white balance on
            "AeEnable":     True,   # auto exposure on
        })

        camera.start()
        # Longer settle time at higher res so AE/AWB fully converge
        time.sleep(1.2)
    ...
    fallback.set(cv2.CAP_PROP_AUTOFOCUS, 1)
    if not fallback.isOpened():
        raise RuntimeError("Pi camera could not be opened")
...
def capture_single_frame():
    with _camera_lock:
        ...
        try:
            ...
            # OpenCV fallback — discard stale buffer frames first
            for _ in range(3):
                _camera_handle.grab()
            ok, frame = _camera_handle.retrieve()
