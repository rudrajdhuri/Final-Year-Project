...
def predict(image_path):
    ...
    def get_model_result():
        if top1_conf >= SPECIES_HIGH_CONF and gap >= SPECIES_GAP:
            return f"✅ Threat - Animal: {predicted_class} ({top1_conf*100:.2f}%)", top1_conf
        ...
        if is_plant_gate_high:
            ...
            disease_confidence = disease_conf.item()
            if disease_confidence >= PLANT_DISEASE_HIGH_CONF:
                return f"❌ No animal detected - Plant image given ({disease_confidence*100:.2f}%)", disease_confidence
            if disease_confidence >= PLANT_DISEASE_MID_CONF and top1_conf < SPECIES_VERY_LOW:
                return f"⚠ No animal detected - Unclear image ({top1_conf*100:.2f}%)", top1_conf
            if pred1_val == 0 and stage1_conf > 0.80:
                return f"❌ Not an animal ({stage1_conf*100:.2f}%)", stage1_conf
            if top1_conf < SPECIES_VERY_LOW:
                return f"⚠ Animal detected but very unclear ({top1_conf*100:.2f}%)", top1_conf
            if gap < SPECIES_GAP:
                return f"⚠ Animal detected but species ambiguous ({top1_conf*100:.2f}%)", top1_conf
            return f"⚠ Unclear image ({top1_conf*100:.2f}%)", top1_conf
        else:
            if pred1_val == 0 and stage1_conf > 0.80:
                return f"❌ Not an animal ({stage1_conf*100:.2f}%)", stage1_conf
            if stage1_conf < 0.60:
                return f"⚠ Uncertain object ({stage1_conf*100:.2f}%)", stage1_conf
            if top1_conf < SPECIES_VERY_LOW:
                return f"⚠ Animal detected but very unclear ({top1_conf*100:.2f}%)", top1_conf
            if gap < SPECIES_GAP:
                return f"⚠ Animal detected but species ambiguous ({top1_conf*100:.2f}%)", top1_conf
