...
//       if (!response.ok || !payload.success) throw new Error(payload.error || "Could not start training");
//       setTrainingStatus(payload.status);
//       onToast("Training started. Drive the full farm path at PWM 200.", "success");
//     } catch (error) {
//       onToast(error instanceof Error ? error.message : "Could not start training.", "error");
//     } finally {
//       setBusy(false);
...
//             <p className="mt-2 text-sm leading-6 text-amber-900 dark:text-amber-100/90 sm:text-base">
//               This page records the path for autonomous replay. The training replay speed stays fixed
//               to PWM 200 so the saved profile matches autonomous playback as closely as possible.
//             </p>
//           </div>
//         </div>
//       </div>
...
//                 </h2>
//                 <p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
//                   Fixed PWM 200, ESP32 path replay, ultrasonic safety on the bot, and both Pi models running together in the background.
//                 </p>
//               </div>
//             </div>
...
function DirectionButton({
  ...
  return (
    <button
      disabled={!enabled}
      onPointerDown={() => {
        if (!enabled) { onLockedClick?.(); return; }
        onPress(command, false);
      }}
      onPointerUp={() => onPress("S", true)}
      onPointerLeave={() => onPress("S", true)}
      onPointerCancel={() => onPress("S", true)}
  ...
  return (
    <div className="rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        <button
          onClick={connected ? onDisconnect : (lockedByOther ? onLockedClick : onConnect)}
          disabled={connecting}
      ...
      <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
        ...
        <div className="hidden justify-center md:flex">
          <div className="grid grid-cols-3 gap-4">
            ...
            <DirectionButton command="S" enabled={!actionDisabled} active={lastCommand === "S"} onPress={sendCommand} onLockedClick={onLockedClick}>
              ■
            </DirectionButton>
            <DirectionButton
              command="R"
            ...
            {allowServo ? (
              <button
                onClick={() => actionDisabled ? onLockedClick?.() : void sendCommand("SERVO")}
                disabled={false}
                className={`flex h-[4.7rem] w-[4.7rem] items-center justify-center rounded-3xl border text-sm font-semibold transition ${
                  actionDisabled || lastCommand !== "S"
                    ? "cursor-not-allowed border-gray-200 bg-gray-100 text-gray-300 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-700"
        ...
        <div className="flex flex-col items-center gap-4 md:hidden">
          <TouchJoystick enabled={!actionDisabled} activeCommand={lastCommand} onCommand={actionDisabled ? (cmd, rel) => { if (!rel) onLockedClick?.(); } : sendCommand} />
          <div className="flex items-center gap-4">
            <MobileCircleButton
              label="Stop"
              onClick={() => actionDisabled ? onLockedClick?.() : void sendCommand("S", true)}
              disabled={false}
              variant="danger"
            />
            {allowServo ? (
...
function ManualPage({
  ...
  // ── UI-only: shown when lockedByOther clicks a greyed-out button ──────────
  const [queueVisible, setQueueVisible] = useState(false);
  const notifyQueue = () => {
    if (!lockedByOther) return;
    setQueueVisible(true);
    setTimeout(() => setQueueVisible(false), 3000);
  };

  return (
    <div className="space-y-5">
      {queueVisible && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800 dark:border-amber-500/20 dark:bg-amber-950/20 dark:text-amber-200">
          You are in the queue. Please wait until the bot is free.
        </div>
      )}
      <ConnectionCard
        connected={connected}
        ...
        <div className="space-y-5 xl:block">
          <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
            ...
            <div className="mt-5 grid gap-4">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <button
                  onClick={lockedByOther ? notifyQueue : () => void startTraining()}
                  disabled={(!lockedByOther && (controlsBlocked || !connected || busy || Boolean(trainingStatus?.active)))}
                ...
                <button
                  onClick={lockedByOther ? notifyQueue : () => void sendCommand("S", true)}
                  disabled={(!lockedByOther && (controlsBlocked || !connected))}
                  className={`rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                    lockedByOther
                      ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
                      : "border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 disabled:opacity-50 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20"
                  }`}
                >
                  Stop Training
                </button>
                <button
                  onClick={lockedByOther ? notifyQueue : () => void saveTraining()}
                  disabled={(!lockedByOther && (controlsBlocked || !connected || busy || !trainingStatus?.active))}
                  className={`rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                    lockedByOther
                      ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
                      : "border border-emerald-200 bg-white text-emerald-700 hover:bg-emerald-50 disabled:opacity-50 dark:border-emerald-500/20 dark:bg-gray-900 dark:text-emerald-300 dark:hover:bg-emerald-950/20"
                  }`}
                >
                  Save Profile
                </button>
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-5">
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <button
                onClick={lockedByOther ? () => { setQueueVisible(true); setTimeout(() => setQueueVisible(false), 3000); } : openTrainingPage}
                className={`rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                  lockedByOther
                    ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button
                onClick={lockedByOther ? () => { setQueueVisible(true); setTimeout(() => setQueueVisible(false), 3000); } : () => void startAutonomous()}
                disabled={!lockedByOther && (working || running || !selectedProfileId)}
                className={`flex min-h-14 flex-1 items-center justify-center gap-2 rounded-3xl px-6 py-4 text-base font-semibold transition ${
                  lockedByOther
                    ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
                    : "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:bg-emerald-600 disabled:opacity-50"
                }`}
              >
                {working && !running ? <Loader2 className="h-5 w-5 animate-spin" /> : <PlayCircle className="h-5 w-5" />}
                Start Autonomous
              ...
              <button
                onClick={lockedByOther ? () => { setQueueVisible(true); setTimeout(() => setQueueVisible(false), 3000); } : () => void stopAutonomous()}
                disabled={!lockedByOther && (working || !running)}
                className={`flex min-h-14 flex-1 items-center justify-center gap-2 rounded-3xl px-6 py-4 text-base font-semibold transition ${
                  lockedByOther
                    ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
                    : "bg-red-500 text-white shadow-lg shadow-red-500/20 hover:bg-red-600 disabled:opacity-50"
                }`}
              >
                {working && running ? <Loader2 className="h-5 w-5 animate-spin" /> : <PauseCircle className="h-5 w-5" />}
                Stop Autonomous
