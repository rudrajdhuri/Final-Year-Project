...
//         src={photoUrl}
//         alt="Latest Pi camera snapshot"
//         className={`aspect-[4/3] w-full object-cover ${highlight ? "ring-4 ring-red-500" : ""}`}
//       />
//     );
//   }
...
//   if (active && showLoader) {
//     return (
//       <div className="flex aspect-[4/3] flex-col items-center justify-center bg-gray-200 text-center dark:bg-gray-800">
//         <Loader2 className="mb-4 h-10 w-10 animate-spin text-gray-500 dark:text-gray-300" />
//         <p className="text-sm font-medium text-gray-600 dark:text-gray-300">
//           Loading next camera snapshot...
//         </p>
...
//         src={uploadedImage}
//         alt="Selected detection preview"
//         className={`aspect-[4/3] w-full object-contain bg-black/90 ${highlight ? "ring-4 ring-red-500" : ""}`}
//       />
//     );
//   }
...
//   const liveSessionRunning = Boolean(status?.running);
//   const ownedLiveSessionRunning = liveSessionRunning && Boolean(status?.owner);
//   // During autonomous mode the backend owns the detection session, not this browser.
//   // We still show the running UI and disable manual controls for everyone.
//   const ownedAutonomousRunning = autonomousRunning;
//   const lockedByOther = Boolean(status?.blocked && !autonomousRunning);
...
//     setManualResult(null);
//     try {
//       // *** FIX: only call this tab's own endpoint ***
//       const endpoint =
//         mode === "animal" ? "/api/animal/detect-animal" : "/api/plant/detect-plant";
//       const response = await apiFetch(endpoint, {
//         method: "POST",
...
//     setCaptureBusy(true);
//     setManualResult(null);
//     // Clear preview immediately so user sees loading state
//     setPreviewImage(null);
//     try {
//       // *** FIX: only call this tab's own capture endpoint ***
//       const endpoint =
//         mode === "animal" ? "/api/animal/capture-camera" : "/api/plant/capture-camera";
//       const response = await apiFetch(endpoint, {
//         method: "POST",
...
//   // *** FIX: auto-detect only calls THIS tab's own model endpoint ***
...
//         {/* Show spinner overlay on image box while capture/upload is processing */}
//         {(captureBusy || detectBusy) && !progressActive && (
//           <div className="mt-3 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
//             <Loader2 className="h-4 w-4 animate-spin" />
//             <span>{captureBusy ? "Capturing from Pi camera..." : "Running detection..."}</span>
//           </div>
//         )}
...
//   const [autonomous, setAutonomous] = useState<AutonomousEnvelope["status"] | null>(null);
//   const sessionId = getClientSessionId();
//   // Snapshots must run whenever autonomous is active (backend started detection,
//   // not the frontend session, so owner=false — but we still need the feed).
//   const sampleActive = Boolean(
//     (statuses?.animal.running && statuses.animal.owner) ||
//       (statuses?.plant.running && statuses.plant.owner) ||
//       autonomous?.running
...
function DetectionTab({
  ...
  // ── UI-only: queue notification when lockedByOther taps a greyed button ──
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-6">
        ...
        <div
          onClick={() => {
            if (lockedByOther) { notifyQueue(); return; }
            if (showIdleControls) fileInputRef.current?.click();
          }}
          onDrop={(e) => {
            e.preventDefault();
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row">
          {showIdleControls ? (
            <>
              <button
                onClick={lockedByOther ? notifyQueue : captureOnce}
                disabled={!lockedByOther && controlsLocked}
                className={`flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                  lockedByOther
              ...
              <button
                onClick={lockedByOther ? notifyQueue : startLiveDetection}
                disabled={!lockedByOther && controlsLocked}
                className={`flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                  lockedByOther
                    ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
              ...
              <button
                onClick={lockedByOther ? notifyQueue : resetState}
                disabled={!lockedByOther && controlsLocked}
                className={`flex min-h-12 items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                  lockedByOther
                    ? "cursor-not-allowed border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500"
                    : "border border-gray-200 bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-60 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
                }`}
              >
                <RefreshCcw className="h-4 w-4" />
                Reset
