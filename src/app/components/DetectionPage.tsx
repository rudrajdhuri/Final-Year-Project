...
//   // Single unified image state — upload OR camera
//   const [image,        setImage]        = useState<string | null>(null);
//   const [imageMode,    setImageMode]    = useState<"upload" | "camera" | null>(null);
//   const [cameraActive, setCameraActive] = useState(false); // true while camera capturing
...
//       setResult(data);
//       // Save to guest history if guest
...
//   // Reconnect if backend already running
//   useEffect(() => {
...
//           setCapturing(false);
//           if (data.last_result) {
//             // image_b64 exposed at top level of status response
...
//         // Show base64 if available, else fallback to URL
//         setImage(data.image_b64 || `${API}/api/${mode}/image/${data.filename}?t=${Date.now()}`);
//         setImageMode("camera");
//         if (isGuest) {
//           pushGuestHistory({ ...data, mode, timestamp: Date.now() });
//         }
//       }
//     } catch {
...
//         {/* Single unified block — click=upload, camera buttons fill it */}
//         <div
...
//         {/* Detect + Reset for uploaded images */}
//         {image && imageMode === "upload" && (
//           <div className="flex gap-2">
//             <button onClick={detectImage} disabled={detectBusy}
//               className="flex-1 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-400 dark:disabled:bg-gray-600 text-white font-medium py-2.5 rounded-lg flex items-center justify-center gap-2 text-sm transition-colors">
//               {detectBusy
//                 ? <><Loader2 className="w-4 h-4 animate-spin" />Detecting...</>
//                 : <><AlertTriangle className="w-4 h-4" />{mode === "animal" ? "Detect Threats" : "Detect Disease"}</>}
//             </button>
...
//         {/* Reset only for camera captures */}
