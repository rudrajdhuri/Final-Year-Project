...
// const tabs: Array<{ key: HistoryTab; label: string; icon: any }> = [
//   { key: "animal", label: "Animal History", icon: AlertTriangle },
//   { key: "plant", label: "Plant History", icon: Leaf },
//   { key: "moisture", label: "Moisture History", icon: Droplets },
//   { key: "temperature", label: "Temperature History", icon: Thermometer },
...
//       (item.temperature >= 35 || item.temperature <= 12)
//     );
//   }
//   return false;
// }
...
//         <div className="mt-4 grid gap-3 sm:grid-cols-2">
//           <div className="rounded-2xl border border-white/70 bg-white/80 px-4 py-3 dark:border-gray-700 dark:bg-gray-900/80">
//             <p className="text-xs uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Result</p>
//             <p className="mt-2 text-sm font-medium leading-6 text-gray-800 dark:text-gray-100">
//               {isAnimal
//                 ? item.message || item.animal_type || "No animal result"
...
//       <div className="mt-4 grid gap-3 sm:grid-cols-2">
//         <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-950/60">
//           <p className="text-xs uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Value</p>
//           <p className="mt-2 text-xl font-bold text-gray-900 dark:text-white">{value}</p>
//         </div>
//         <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-950/60">
//           <p className="text-xs uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">Meaning</p>
//           <p className="mt-2 text-sm font-medium leading-6 text-gray-800 dark:text-gray-100">{message}</p>
//         </div>
//       </div>
//     </div>
//   );
...
//     Record<"animal" | "plant", DetectionItem[]>
//   >({ animal: [], plant: [] });
//   const [sensorRecords, setSensorRecords] = useState<SensorItem[]>([]);
//   const [loading, setLoading] = useState(true);
...
//     // moisture / temperature / humidity all share the same sensorRecords array.
//     // SensorCard reads the correct field (item.moisture / item.temperature / item.humidity)
//     // based on the tab prop passed to it.
//     return sensorRecords;
//   }, [detectionRecords, sensorRecords, tab]);
...
/**
 * Parse a timestamp correctly regardless of whether it carries timezone info.
 *
 * The Flask backend stores datetime objects via pymongo without an explicit
 * timezone. When serialised they come out as plain ISO strings like
 * "2026-05-14T11:22:56.123000" (no Z, no +05:30). JavaScript's Date.parse()
 * treats bare strings as UTC, so a time that was actually 11:22 IST gets
 * displayed as 16:52 IST after the +5:30 shift.
 *
 * Fix: if the string has no timezone marker (no Z, no +, no trailing offset)
 * we append "+05:30" so JS parses it as the IST instant it really is.
 * Strings that already carry timezone info (from the frontend / guest history)
 * are left untouched.
 */
function parseTimestamp(value?: string | number | null): Date | null {
  if (!value) return null;

  // Numeric millisecond epoch (guest history pushes Date.now())
  ...
  // No timezone → backend sent a naive IST datetime → append +05:30
