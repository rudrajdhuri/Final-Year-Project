...
//       const response = await fetch("http://127.0.0.1:5000/api/plant/detect-plant", {
//         method: "POST",
//         headers: {
...
//                     onClick={detectPlant}
//                     disabled={detecting}
//                     className="flex-1 bg-green-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-green-700 disabled:bg-gray-400 transition-colors flex items-center justify-center gap-2"
//                   >
...
//                     disabled={detecting}
//                     className="bg-gray-100 text-gray-900 px-6 py-3 rounded-lg"
//                   >
//                     Reset
//                   </button>
...
//         {/* <div className="bg-blue-50 border rounded-lg p-5 mt-6">
...
//             <li>User uploads a plant leaf image from the field.</li>
//             <li>The system preprocesses the image using OpenCV.</li>
//             <li>The trained deep learning model analyzes disease patterns.</li>
//             <li>The model predicts the plant disease class.</li>
//             <li>Detection results are stored in the database for history tracking.</li>
//           </ul>
//         </div>        
//       </div>
...
// import { useState, useRef } from "react";
// import { Upload, AlertTriangle, CheckCircle, Loader2, HelpCircle } from "lucide-react";
...
//     try {
//       const res = await fetch("http://127.0.0.1:5000/api/plant/detect-plant", {
//         method: "POST",
...
//   const isHealthy  = msg.includes("HEALTHY")  || msg.includes("No Threat Detected");
...
//             Upload a plant leaf image to detect diseases and health status
//           </p>
//         </div>
...
//                     : <><AlertTriangle className="w-4 h-4" /> Detect Disease</>}
//                 </button>
//                 <button
//                   onClick={reset}
//                   disabled={detecting}
...
//             <li>Confidence thresholds prevent low-quality predictions.</li>
...
import { useState, useRef } from "react";
import { Upload, Camera, AlertTriangle, CheckCircle, Loader2, HelpCircle } from "lucide-react";
...
export default function PlantDetection() {
  ...
  const captureCamera = async () => {
    ...
    setSelectedImage(null);
    try {
      const res = await fetch("http://127.0.0.1:5000/api/plant/capture-camera", {
        method: "POST",
      });
      ...
      if (data.success && data.filename) {
        setSelectedImage(`http://127.0.0.1:5000/api/plant/image/${data.filename}`);
      }
    } catch {
    ...
    <div className="min-h-full bg-gray-50 dark:bg-gray-950 transition-colors duration-200 p-4 sm:p-6 lg:p-8">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Upload a plant leaf image or capture from camera to detect diseases
          </p>
        </div>
